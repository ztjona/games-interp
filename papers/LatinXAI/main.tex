...
\title{The Wall Is Disjunction, Not Abstraction:\\
       Sparse Autoencoders on a Game-Playing Network}
...
\begin{abstract}
  Sparse autoencoders (SAEs) trained on game-playing networks reliably
  recover per-cell board facts and reliably fail on relational concepts such
  as ``this line is one move from completion.'' Whether the network lacks the
  concept or the dictionary cannot name it is undecidable from one layer with
  nothing to compare against. We measure a linear probe and an SAE dictionary
  on the same activations at two layers of a Quarto agent, plus a third
  measurement of where a concept goes when no single atom carries it. Across
  three agents, three SAE architectures, four dictionary widths and 546
  ground-truth concepts: (i) the preferred layer partitions by concept
  family, so a pooled comparison averages effects of opposite sign and
  reports nothing; (ii) the probe overturns the natural reading of that
  split, since for state threats the convolutional layer is equally or more
  linearly available in 11 of 12 cells while its dictionary isolates
  $1.4$--$4.8\times$ less; and (iii) the residual failure has a specific
  logical shape. Holding basis, agent-relativity and base rate fixed and
  varying only logical form, single-atom capture drops from 81.2\% to 0.0\%
  the moment a concept is a disjunction; agent-relativity, the property we
  had previously blamed, costs 2 points. The dictionary instead rebuilds each
  4-way disjunction from exactly its four disjuncts (modal cost 4 latents,
  and none of 266 concepts is cheaper), and an 8$\times$ larger dictionary
  does not move it. The wall is not missing information and not capacity; it
  is the absence of an atom for OR.
\end{abstract}
...
what was available to be decomposed. Our first step is a measurement that is
...
we add a second measurement. When no single atom carries a concept the signal is
...
\begin{enumerate}
  ...
  \item \textbf{The mechanism, read directly, plus a capacity control}
        ...
        below 4), and growing the dictionary 8$\times$ (4{,}096 $\to$ 32{,}768) leaves
        the same 22 of 23 concepts spread.
\end{enumerate}
...
is an information limit, $I$ small with $A$ large a factorisation limit, and
within the latter, $S$ small with $R$ small means the dictionary never picked the
concept up while $S$ small with $R$ large means it split it. Two caveats hold
throughout: everything here is \textbf{decodability, not causality} (matching is
greedy argmax, so the matched latent may be a spectator), and $A$ bounds
\emph{linear} availability while $R$ is measured over 64 of the 4{,}096 latents,
so both lower-bound presence.
...
\begin{table}[t]
  ...
  \caption{Best-latent MCC by concept family, averaged over nine matched
    ...
    wins'' counts cells with $\Delta>0$. The partition is total: all fifteen
    cell-averaged gaps point the predicted way, the trivial-baseline family
    only marginally ($-0.008$).
    $\ddagger$ base rate far from $p_{\text{ref}}$; read on raw MCC.}
  \label{tab:split}
  \small
  \input{table_split.tex}
\end{table}
...
which reverses on M3 and still does so under the repaired conv2 dictionary of
\S\ref{sec:robustness}. The methodological consequence is that a \emph{pooled}
...
This is the first observation. The information is not lost crossing the
...
isolation alone should not be used to choose a fix. A third case behaves as the
information account predicts: at \texttt{conv2}, agent-relative concepts score
low on \emph{availability} too ($0.338$ against $0.808$ at fc1, M3's ``winnable
square'' family). When a layer genuinely lacks a concept availability drops as
well, and state threats do not. Everything below
...
point of view. A basis fixes logical form, agent-relativity and prevalence at
once. The comparison that separates them is
...
prediction, 21.6\% at 20 or beyond), the expected outcome for a concept that ORs
...
same 22 of 23 agent-relative threat concepts come out \emph{spread} at every
width, with median concentration flat at $0.19$--$0.20$.
...
overlap the rows $R^2$ is measured on is double-dipping the permutation null
cannot see, since it shuffles labels \emph{after} selection; a selection-aware
null (permute first, then refit the whole pipeline) returns $-0.0006$ to
$-0.0013$ where the real values are $0.058$--$0.656$, so selecting 64 of the
199--643 live latents buys no held-out $R^2$ from noise.
...
concepts probe best in final layers and long-term ones near mid-depth, and
\citet{du2025gpt} report the SAE-based analogue in OthelloGPT. Both compare
\emph{probes} (or dictionaries) across layers; what we add is the comparison of
\emph{probe and dictionary on the same activations}, which separates ``the layer
knows it'' from ``the dictionary can name it.'' Under a probe-only view
\S\ref{sec:availability} is invisible: the probe alone says both layers carry
threats, the dictionary alone says only the bottleneck does.
...
pinned atoms are the ones the policy consumes is untested. \textbf{(ii)
  Retention is measured over 64 of 4{,}096 latents}: a concept recoverable only
from hundreds of latents
