...
\begin{abstract}
  ...
  perfectly by concept family---every per-cell family prefers the convolutional
  layer, every relational family the bottleneck, in 9 of 9 matched cells---so a
  pooled comparison averages effects of opposite sign and reports nothing;
  ...
  available in 11 of 12 cells while its dictionary isolates 3--5$\times$ less;
...
cheap and, as far as we can tell, not standard: run a linear probe \emph{and} an
...
\begin{enumerate}
  \item \textbf{The preferred layer partitions perfectly by concept family}
        (\S\ref{sec:partition}): over nine matched (agent $\times$ recipe) cells,
        every per-cell family prefers the convolutional layer and every relational
        family the bottleneck, with no exceptions. A pooled comparison averages a
        $+0.28$ against a $-0.12$ and is uninterpretable.
  \item \textbf{The gap is factorisation, not information}
        ...
        dictionary isolates 3--5$\times$ less.
  \item \textbf{Retention separates two failures that score alike}
        (\S\ref{sec:retention}): at the strongest agent's bottleneck the dictionary
        holds $15.2\times$ the untrained-network floor and concentrates $0.92$ of it;
        at its convolutional layer, $2.3\times$ and $0.13$. Both score
        $I \approx 0.18$ on the concepts we care about, for opposite reasons.
...
\paragraph{Agents.} Three value-based convolutional agents share an identical
trunk and differ only in training procedure: \textbf{M1} distils a depth-2
minimax selector, \textbf{M2} adds an oracle-ablation curriculum, and
\textbf{M3} adds dense piece-safety shaping. Head-to-head, $\text{M3} >
...
\subsection{The preferred layer partitions perfectly by concept family}
\label{sec:partition}
...
\begin{table}[t]
  ...
  \caption{Best-latent MCC by concept family, averaged over nine matched
    ...
    wins'' counts cells with $\Delta>0$. The partition is total: every per-cell
    family goes to conv2, every relational family to fc1.
...
agents and three recipes. The methodological consequence is that a \emph{pooled}
...
model. Our own earlier pooled reading of these same runs said simply ``fc1 is
better,'' which is true on the menu average and false for a third of the menu.
...
\begin{table}[t]
  ...
  \caption{Availability (linear probe) vs.\ isolation (best latent) on state
    ...
    linearly available in 11 of 12 cells, while fc1 isolates 3--5$\times$ more.}
  \label{tab:avail}
  \small
...
factored into individual directions at \texttt{fc1}. The two dictionaries have
identical input width, size, recipe, positions and concepts, so the only
variable left is how the layer arranges the information. Note also how flat the
conv2 efficiency column is: $0.18$--$0.23$ across every
basis, family and agent, so whatever prevents atomisation there is insensitive
...
a factorisation failure in the sense \S\ref{sec:method} defines. The two are
indistinguishable on isolation alone---both score $I \approx 0.18$---which is
why isolation alone should not be used to choose a fix. A third case completes
...
($0.338$, against $0.808$ at fc1 on M3's ``winnable square'' family), because
they are functions of the offered piece and the offered piece is first combined
with board state at the fully-connected layers. When a layer genuinely lacks a
...
\textbf{Nor is it prevalence, which runs the wrong way.} The \emph{most}
captured group is the rarest (agent-relative pinned, base rate $0.0029$) and the
least captured is among the most common (disjunctive, $0.0316$); restricting
both arms to a common window $[0.015, 0.050]$ leaves the effect intact, 71.4\%
captured for pinned ($n=868$) against \textbf{0.0\%} for disjunctive ($n=437$).
Critically, \textbf{the disjunctive concepts are not information-poor}
(Figure~\ref{fig:disj}, left): they have the \emph{highest} retention in the
table---median $R^2 = 0.741$ for \textsc{count}'s 4-way disjunctions against
$0.530$ for its pinned concepts. The dictionary knows the answer and has no atom
for it.
...
\texttt{spread} 74.6\%, \texttt{absent} 6.8\%), including an entire unsupervised
basis at 85.5\% on M3 / \texttt{fc1}; the supervised positive control---slots
...
selection-aware null---permute first, then re-rank, re-select and refit---
...
threats. All conv2 numbers above use the repaired dictionary.
...
A low SAE score is not self-interpreting. On these agents the same
$I \approx 0.18$ means three different things: ``barely present in the layer''
(agent-relative concepts at \texttt{conv2}), ``present in the layer, absent from
the dictionary'' (state threats at \texttt{conv2}), and ``present in the
dictionary, split across its disjuncts'' (disjunctive concepts at \texttt{fc1}).
Availability separates the first from the other two, retention the second from
the third, and only the third is a failure dictionary design can address.
...
dictionary having resolved atoms at all, and holds on one (agent, layer) pair. \textbf{(iv) Linear
  availability only}, and two layers rather than a depth sweep---a concept
present non-linearly registers as unavailable, and where between the two layers
the change of form occurs is open. \textbf{(v) One game, one architecture
