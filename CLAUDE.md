...
| `runners/<slug>.sh` | Committed, self-contained runner, one per champion or analysis (`champYb.sh`, `3A-dilution.sh`). Full workflow end-to-end: data/train -> eval -> **export** -> **stage plan**. `cd`s to repo root; verifies inputs before running. Runs on Deep Brain. | One per unit of reproducible work. Always end by folding in the relevant `export_*.py` and calling `scripts/emit_stage.py --slug=<slug>` so it writes `stage_<slug>.md`. | Update in place; committed as the reproducibility record. |
...
Reproducible workflows live in `runners/` (one committed, self-contained script
per champion/analysis; run on Deep Brain). Each ends by folding in the relevant
`export_*.py` and writing a `stage_<slug>.md` commit plan:
```bash
bash runners/champYb.sh                     # data->LP->SAE->eval->export->stage
DRY_RUN=1 bash runners/3A-dilution.sh       # analysis runner; check inputs first
```
