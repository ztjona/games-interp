...
| `runners/<slug>.ps1` | Committed, self-contained **PowerShell 7** runner, one per champion or analysis (`champYb.ps1`, `3A-dilution.ps1`). Full workflow end-to-end: data/train -> eval -> **export** -> **stage plan**. `cd`s to repo root; fail-fast (`$PSNativeCommandUseErrorActionPreference`); parallelizes only file-disjoint stages (position-gen, activation, SAE *training*) across all GPUs, **eval sequential** (shared registry/_h). Runs on Deep Brain (Win PS 7; no bash). Launch detached via `runners/launch.ps1 <slug>`. | One per unit of reproducible work. Always end by folding in the relevant `export_*.py` and calling `scripts/emit_stage.py --slug=<slug>`, which writes `stage_<slug>.md` only if an output needs `git add -f`. | Update in place; committed as the reproducibility record. |
...
| `stage_<slug>.md` (repo root) | Written by a runner's final `emit_stage` step **only when an output needs `git add -f`** (a git-ignored keeper, e.g. an Atlas-shipped checkpoint); ordinary outputs show in `git status`, bulk ignored files stay out, and a stale plan is removed. | Regenerated every run; not hand-edited. | Ephemeral (gitignored); safe to delete. |
...
folding in the relevant `export_*.py` and `emit_stage` (a `stage_<slug>.md` appears only when a file needs `git add -f`).
...
# lists exactly the Atlas-shipped checkpoints to force-add in stage_<slug>.md;
# bulk activations/_h caches are git-ignored and simply stay out.
```
...
Phase 3B-causal (interchange interventions). Wave 1 (rule `3B.C1`): the
2026-09-12 pre-registration + amendments; Wave 1b (rule `3B.C2`, network-own
targets, fresh `gold` position sets): the 2026-09-14 pre-registration; Wave 1c
(rule `3B.C3`: installation is the verdict, removal tested by DAS-k / R7-off, on
second draws `gold3r2`/`gold5r2`): the 2026-09-15 pre-registration.
**Portable: a champion is a config file**, `configs/3B-causal/champ<Tag>.yaml`,
and so is a position set, `champ<Tag>-<set>.yaml` (`extends:` the champion's,
sets `rule:` and a `positions_recipe`); nothing in the pipeline names either:
```powershell
...
pwsh -File runners\launch.ps1 3B-causal -Champ Yb     # Wave 1 (~55 min); refuses unless design files are committed
pwsh -File runners\launch.ps1 3B-causal -Set gold3r2,gold5r2 -DryRun   # Wave 1c: builds missing sets, + freshness, feasibility
pwsh -File runners\launch.ps1 3B-causal -Set gold3r2,gold5r2           # Wave 1c: both sets in parallel, one GPU each
python scripts/build_gold_sets.py --config=configs/3B-causal/champYb-gold3.yaml --dry-run   # the build commands
```
