...
- **Q-values on ILLEGAL actions are not preferences — score over legal actions only.** The Quartopy engine *retries* an invalid move (up to 16 times, via `ith_option`) instead of penalising it, and the S4 bots walk the network's ranking to the first valid entry, so Q of an occupied cell / unavailable piece is never a training target. champYb's raw place argmax is on an occupied cell **67.2%** of the time (chance 34.2%) — so "the intervention pushed mass onto an illegal move" is baseline behaviour, not an effect. Any analysis of Q-values, rankings or interventions restricts to the legal set; counterfactual inputs should keep the legal set identical. See [`docs/diary/2026-09-12_3B-causal-preregistration.md`](docs/diary/2026-09-12_3B-causal-preregistration.md) §2.
